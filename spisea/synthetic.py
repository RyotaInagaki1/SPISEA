...
class Binary_Cluster(Cluster):
    ...
    def make_singles_systems_table(self, isMulti, sysMass):
        """
        Finds the stars in the isochrone that best
        match the single star systems generated by the IMF
        and puts that star in the isochrone in
        an AstroPy table, given that the initial mass
        of the star is within 10% of the IMF generated counterpart's.
        Input: isMulti (array of booleans)-> Whether a star system is a
        multi-star system. (output of the IMF)
        sysMass (array of floating points)->
        System masses of all stellar systems (generated by IMF)
        Output: AstroPy table that contains all
        information regarding single star systems.
        Data values, including initial mass, come
        from rows of the self.isochrone
    ...
    def make_primaries_and_companions(self, star_systems, compMass):
        ...
        compMass--> list of lists of masses of companions for each star system
        Output: Creates tables star_systemsPrime and companions.
        The former contains properties of the primary star
        of the multistar system and of the photometry and initial mass of
        system as a whole.
        The latter contains properties of the secondary and higher
        order companions in each star system.
...
class Isochrone_Binary(Isochrone):
    """
    Isochrone class incorporating binaries
    ...
    iso_dir : path, optional
         Path to isochrone directory. As of Jan 11, 2021 not functional.
    wave_range : list, optional
        length=2 list with the wavelength min/max of the final spectra.
        Units are Angstroms. Default is [3000, 52000].
    min_mass : float or None, optional
    ...
    def __init__(self, logAge, AKs, distance, metallicity,
                 ...
                 wd_atm_func=default_wd_atm_func,
                 wave_range=[3000, 52000], red_law=default_red_law,
                 mass_sampling=1, iso_dir='./',
                 min_mass=None, max_mass=None,rebin=True, recomp=True,
        ...
        # log(a)
        primaries = copy.deepcopy(singles)
        ...
        self.name_to_list = {"Singles": self.spec_list_si,
                          "Primaries": self.spec_list2_pri,
                          "Secondaries": self.spec_list3_sec}
        table_dic = {"Singles": singles, "Primaries": primaries,
                    "Secondaries": secondaries}
        self.table_pairings = table_dic
        # For each temperature extract the synthetic photometry.
        for x in table_dic:
            tab = table_dic[x]
            atm_list = self.name_to_list[x]
            ...
            # the secondary star models that are not 101 type
            # These will be in a limbo-state for Fall 2020 release
            if x == "Primaries":
        ...
        # extract the synthetic photometry if possible.
            for ii in range(len(tab)):
                ...
                if 'Secondaries' == x and tab['merged'][ii]:
